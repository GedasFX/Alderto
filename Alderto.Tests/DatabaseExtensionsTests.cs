...
namespace Alderto.Tests
{
    public class DatabaseExtensionsTests
    {
        ...
        public async Task GetMember()
        {
            ...
            var member = await _context.GetGuildMemberAsync(guildId: 1, memberId: 1, addIfNonExistent: false);
            Assert.Null(member);

            // This time make sure to add the member.
            member = await _context.GetGuildMemberAsync(guildId: 1, memberId: 1);
